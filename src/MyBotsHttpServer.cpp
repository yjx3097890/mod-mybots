...
std::string HeaderValue(std::string const& headers, std::string const& key)
{
    ...
    if (pos == std::string::npos)
    {
        ...
        for (char& c : lowerHeaders)
            c = static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
        pos = lowerHeaders.find(lowerKey + ":");
        if (pos == std::string::npos)
            return {};
    }
...
void HandleRequest(boost::asio::ip::tcp::socket sock)
{
    try
    {
        ...
        {
            Send(sock, 200, "{\"ok\":true,\"service\":\"mod-mybots\",\"version\":\"0.1.0\"}");
            return;
        }
        ...
        consumeCharacterPath("/v1/characters/");
        ...
        bool postSelfbot = (method == "POST") && (path.find("/selfbot") != std::string::npos);
        if (method == "GET" && !postSelfbot)
            op = MyBotsIntentOp::Status;
        else if (postSelfbot)
        {
            bool enabled = true;
            if (!body.empty())
            {
                auto const pos = body.find("enabled");
                if (pos != std::string::npos)
                {
                    auto const t = body.find("true", pos);
                    auto const f = body.find("false", pos);
                    if (f != std::string::npos && (t == std::string::npos || f < t))
                        enabled = false;
                }
            }
            op = enabled ? MyBotsIntentOp::SelfbotOn : MyBotsIntentOp::SelfbotOff;
        }
        else
        {
        ...
        auto intent = sMyBotsIntentQueue.Submit(op, name, guidLow);
        if (!sMyBotsIntentQueue.Wait(intent, sMyBotsConfig.ApiTimeoutMs()))
        {
            Send(sock, 503, "{\"ok\":false,\"code\":\"timeout\",\"message\":\"World thread did not process the intent in time\"}");
            return;
        }

        Send(sock, intent->httpStatus, intent->response);
    }
